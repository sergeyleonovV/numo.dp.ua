	                ...
	                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?> role="article">
                        <div class="singlebox our_authors">
							    ...
							    <section class="entry-content clearfix">
							        <?php the_content(); ?>
									    <?php $candour_guestauthors = get_users('orderby=post_count&role=contributor');
                                        if(isset($candour_guestauthors) && !empty($candour_guestauthors)){ 
				    foreach($candour_guestauthors as $author){
		                    ...
		                    <div class="item ">
                                <div class="desc center">
                                    <h6 class="authorName"><?php echo $author->display_name; ?></h6>
    			                        <p class="email"><a href="<?php echo get_author_posts_url( $author->ID ); ?>"><?php _e('View Posts', 'candour'); ?></a></p>                
                                </div>
                                <span class="line" style="width: 0px;"></span> 
                                <?php echo get_avatar( $author->user_email, '128' ); ?>                                   
                            </div> <?php }}} ?>
										
						    <?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'candour' ), 'after' => '</div>' ) ); ?>
							    <div class="clr"></div>
								</section> <!-- end section -->
							    <footer class="article-footer">
								    <?php edit_post_link( __( 'Edit', 'candour' ), '<span class="edit-link">', '</span>' ); ?>
							    </footer> <!-- end footer -->
